...
Generate answers using functions in `agents/generator_agent.py`. Synthesizes information from graded-relevant documents or provides fallback when no context available.
...
**Implementation:** `agents/generator_agent.py`, uses `GENERATION_MODEL` from `config.py` (default: Haiku 4.5), temperature 0.3, max tokens 2000.
