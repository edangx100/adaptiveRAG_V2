...
Rewrite queries using `rewrite_query()` in `agents/query_agent.py`. Typically triggered when `len(relevant_docs) == 0` during adaptive retry.
...
**Implementation:** `agents/query_agent.py`, uses `REWRITE_MODEL` from `config.py` (default: Haiku 4.5), temperature 0.3.
