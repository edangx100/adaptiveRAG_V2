...
Grade documents for relevance using the functions in `agents/grader_agent.py`. The grading uses Claude API with structured outputs for consistent binary decisions.
...
**Implementation:** `agents/grader_agent.py`, uses `GRADING_MODEL` from `config.py` (default: Haiku 4.5), temperature 0.
